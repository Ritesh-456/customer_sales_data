...
select shipping_type, round(avg(purchase_amount),2) as "Avgerage Purchase amount"
from customer
where shipping_type in ('Standard', 'Express')
group by shipping_type

-- q5 do subscribed customer spend more? compare average spend and total revenue between subscribers and non-subscriber
-- Answer = NO

select count(customer_id) as "Total Customer", subscription_status, round(avg(purchase_amount),2) as avg_spend, sum(purchase_amount) as revenue
from customer
group by subscription_status
order by avg_spend, revenue desc;

-- q6 which 5 products have the highes percentage of purchase wiith discount applied
...
-- q7 segment customers into new, returning and loyal based on their total number of previous purchase, and show the count 
